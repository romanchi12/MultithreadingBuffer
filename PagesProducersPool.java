 ...
 */
public class PagesProducersPool {
    private ArrayList<PagesProducer> pagesProducers;
    int MAX_PAGES_PRODUCERS;
    public PagesProducersPool(PagesBuffer pagesBuffer){
        this(pagesBuffer,4);
    }
    public PagesProducersPool(PagesBuffer pagesBuffer, int MAX_PAGES_PRODUCERS){
        ...
        for(int i = 0; i < this.MAX_PAGES_PRODUCERS; i++){
            pagesProducers.add(new PagesProducer(pagesBuffer));
        }
    }
